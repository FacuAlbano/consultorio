...
import { useLoaderData, useActionData, useSearchParams, Form, useFetcher, useRevalidator, Link } from "react-router";
import { toast } from "sonner";
import type { Route } from "./+types/dashboard.agenda.eliminar";
...
import { listGeneratedAgendaBlocks, deleteGeneratedAgendaBlock, deleteGeneratedAgendaBlocksByIds } from "~/lib/generated-agenda.server";
import { Card, CardContent, CardHeader, CardTitle } from "~/components/ui/card";
import { Button } from "~/components/ui/button";
import { Input } from "~/components/ui/input";
import { Label } from "~/components/ui/label";
import { Calendar, Clock, ArrowLeft, Loader2, Trash2 } from "lucide-react";
...
const INTENT_DELETE_ONE = "deleteOne";
const INTENT_DELETE_ALL = "deleteAll";
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === INTENT_DELETE_ONE) {
    const blockId = formData.get("blockId") as string;
    const result = await deleteGeneratedAgendaBlock(blockId);
    if (!result.success) return { success: false, error: result.error };
    return { success: true, deleted: 1 };
  }

  if (intent === INTENT_DELETE_ALL) {
    const idsStr = formData.get("blockIds") as string;
    const blockIds = idsStr ? idsStr.split(",").filter(Boolean) : [];
    const result = await deleteGeneratedAgendaBlocksByIds(blockIds);
    if (!result.success) return { success: false, error: result.error };
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Filtre por médico, rango de fechas y horario. Use &quot;Mostrar Turnos&quot; para ver los bloques y eliminarlos.
        </p>
      </div>
      ...
      {blocks.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <p className="text-muted-foreground text-sm">Puede eliminar uno por uno o todos los mostrados.</p>
          </CardHeader>
          <CardContent className="space-y-4">
            <fetcher.Form method="post">
              <input type="hidden" name="_intent" value={INTENT_DELETE_ALL} />
              <input type="hidden" name="blockIds" value={blocks.map((b) => b.block.id).join(",")} />
              <Button type="submit" variant="destructive" className="gap-2" disabled={fetcher.state !== "idle"}>
                {fetcher.state !== "idle" ? (
                  <><Loader2 className="h-4 w-4 animate-spin" /> Eliminando...</>
                ) : (
                  <>
                    <Trash2 className="h-4 w-4" />
                    Eliminar todos los mostrados ({blocks.length})
                  </>
                )}
              </Button>
            </fetcher.Form>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left py-2 px-2 w-24">Acción</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={block.id} className="border-b border-border/50 hover:bg-muted/30">
                      ...
                      <td className="py-2 px-2">
                        <fetcher.Form method="post" className="inline">
                          <input type="hidden" name="_intent" value={INTENT_DELETE_ONE} />
                          <input type="hidden" name="blockId" value={block.id} />
                          <Button type="submit" variant="ghost" size="sm" className="text-destructive hover:text-destructive gap-1 h-8" disabled={fetcher.state !== "idle"}>
